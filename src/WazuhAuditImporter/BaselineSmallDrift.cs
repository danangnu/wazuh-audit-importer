...
public sealed record Step17SmallDriftReview(
    string CandidateId,
    ...
    bool EligibleForStep17Approval,
    IReadOnlyList<SolrPathComparison> Missing,
    IReadOnlyList<SolrPathComparison> Stale,
    string Detail);

public static class Step17SmallDriftPolicy
{
    private static readonly HashSet<string> InitialPilotCandidates = new(StringComparer.Ordinal)
    {
        "1180002", "1180007", "1180009"
    };

    public static IReadOnlyCollection<string> InitialPilotBatch => InitialPilotCandidates;

    public static bool IsPilotCandidate(string candidateId) => InitialPilotCandidates.Contains(candidateId);

    public static void RequirePilotCandidate(string candidateId)
    {
        if (!IsPilotCandidate(candidateId))
            throw new EventConflictException(
                $"Step 17 initial SmallDrift pilot is limited to candidates {string.Join(',', InitialPilotCandidates.OrderBy(x => x, StringComparer.Ordinal))}; candidate {candidateId} is outside this controlled batch.");
    }
    ...
    {
        RequirePilotCandidate(stored.CandidateId);
        var e = live.Evidence;
        ...
        {
            throw new EventConflictException(
                $"Step 17 simple SmallDrift policy rejected candidate {stored.CandidateId}: " +
                ...
                "The initial pilot requires exactly one missing current document and one stale Solr document, with no other conflicts.");
        }
    }
}

public static class Step17SmallDriftService
{
    public static int Review(
        ...
        string? candidateId)
    {
        settings.ValidateWorker(workerRoot, Path.Combine(Path.GetTempPath(), "wazuh-step17-small-drift-review-validation"));
        var ids = candidateId is null
            ? Step17SmallDriftPolicy.InitialPilotBatch.OrderBy(x => x, StringComparer.Ordinal).ToArray()
            : new[] { candidateId };
        var rows = new List<Step17SmallDriftReview>();

        Console.WriteLine("=== Step 17 controlled SmallDrift review ===");
        Console.WriteLine("Initial batch: 1180002,1180007,1180009 only.");
        ...
        foreach (var id in ids)
        {
            ...
            Step17SmallDriftPolicy.RequirePilotCandidate(id);
        ...
        Console.WriteLine("STEP 17 SMALLDRIFT REVIEW COMPLETE. Read/report only; no baseline approval and no Solr write occurred.");
        return 0;
    }
    ...
    public static BaselineEnrollmentSnapshot Approve(
        ...
        bool apply)
    {
        settings.ValidateWorker(workerRoot, Path.Combine(Path.GetTempPath(), "wazuh-step17-small-drift-approval-validation"));
        settings.ValidateAllowedCandidate(candidateId);
        Step17SmallDriftPolicy.RequirePilotCandidate(candidateId);
        ...
        Step17SmallDriftPolicy.RequireSimpleOneForOne(stored, live);
        var review = BuildReview(stored, live);

        Console.WriteLine("Step 17 simple SmallDrift approval preflight.");
        PrintReview(review);
        ...
        if (!acknowledgeSmallDrift)
            throw new FormatException("Step 17 --apply requires --ack-small-drift to confirm the one-missing/one-stale evidence was explicitly reviewed.");

        var defaultNote = $"Step 17 reviewed simple SmallDrift enrollment; missing={live.Evidence.MissingCount}; stale={live.Evidence.StaleCount}; baseline_sha256={stored.BaselineSha256}";
        ...
        Console.WriteLine($"\nSTEP 17 SMALLDRIFT BASELINE APPROVED. candidate={candidateId}; approved_by={approved.ApprovedBy}; sha256={approved.BaselineSha256}");
    ...
    private static Step17SmallDriftReview BuildReview(
        ImportSettings settings,
        MySqlConnection connection,
        string workerRoot,
        string candidateId)
    ...
    public static Step17SmallDriftReview BuildReview(
        BaselineEnrollmentSnapshot stored,
        BaselineEnrollmentCapture live)
    {
        var e = live.Evidence;
        var same = string.Equals(stored.BaselineSha256, live.BaselineSha256, StringComparison.OrdinalIgnoreCase);
        var eligible = Step17SmallDriftPolicy.IsPilotCandidate(stored.CandidateId) &&
                       Step17SmallDriftPolicy.IsSimpleOneForOneSmallDrift(stored.Status, same,
        ...
        var detail = eligible
            ? "Eligible for the initial Step 17 one-for-one SmallDrift approval after exact operator review."
            : "Not eligible for the initial Step 17 approval; keep baseline gated.";
        return new Step17SmallDriftReview(stored.CandidateId, stored.BaselineSha256, same,
    ...
    private static void PrintReview(Step17SmallDriftReview row)
    {
        Console.WriteLine($"candidate={row.CandidateId} sha256={row.BaselineSha256} fingerprint_match={row.FingerprintMatch.ToString().ToLowerInvariant()} " +
                          ...
                          $"missing={row.MissingCount} stale={row.StaleCount} other={row.OtherConflictCount} step17_approval={(row.EligibleForStep17Approval ? "YES" : "NO")}");
    ...
    private static void SaveReports(string? reportDirectory, IReadOnlyList<Step17SmallDriftReview> rows)
    {
        if (string.IsNullOrWhiteSpace(reportDirectory)) return;
        var full = Path.GetFullPath(reportDirectory);
        Directory.CreateDirectory(full);
        var stamp = DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture);
        var jsonPath = Path.Combine(full, $"step17-small-drift-review-{stamp}.json");
        var csvPath = Path.Combine(full, $"step17-small-drift-review-{stamp}.csv");
        var mdPath = Path.Combine(full, $"step17-small-drift-review-{stamp}.md");

        File.WriteAllText(jsonPath, JsonSerializer.Serialize(new { GeneratedAtUtc = DateTime.UtcNow, Rows = rows },
            new JsonSerializerOptions { WriteIndented = true }));

        var csv = new StringBuilder();
        csv.AppendLine("candidate_id,baseline_sha256,fingerprint_match,eligible,solr,match,missing,stale,other,step17_approval");
        foreach (var r in rows)
            csv.AppendLine(string.Join(',', new[]
            {
                ...
                Csv(r.EligibleForStep17Approval ? "yes" : "no")
            }));
        File.WriteAllText(csvPath, csv.ToString());

        var md = new StringBuilder();
        md.AppendLine("# Step 17 SmallDrift Review");
        md.AppendLine();
        md.AppendLine("Controlled initial batch: `1180002`, `1180007`, `1180009`. Review is read-only. Approval does not itself change Solr.");
        foreach (var r in rows)
        {
            md.AppendLine();
            ...
            md.AppendLine($"- Step 17 approval eligible: `{r.EligibleForStep17Approval}`");
