    ...
    public static int Run(string[] args)
    {
        try
        {
            ...
            while (i < args.Length)
            {
                ...
                if (option == "--apply")
                {
                    if (command is not ("import" or "solr-execute" or "baseline-approve" or "baseline-approve-clean" or "baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift" or "baseline-approve-step20-missing-only")) throw new FormatException("--apply is only valid with import, solr-execute or a controlled baseline approval.");
                    apply = true;
                }
                else if (option == "--ack-small-drift")
                {
                    if (command is not ("baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift")) throw new FormatException("--ack-small-drift is only valid with a controlled SmallDrift approval.");
                ...
                {
                    if (command is not ("baseline-approve" or "baseline-approve-clean" or "baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift" or "baseline-approve-step20-missing-only")) throw new FormatException("--baseline-sha256 is only valid with baseline approval commands.");
                    if (i == args.Length || args[i].StartsWith("--", StringComparison.Ordinal))
                ...
                {
                    if (command is not ("baseline-approve" or "baseline-approve-clean" or "baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift" or "baseline-approve-step20-missing-only")) throw new FormatException("--reviewer is only valid with baseline approval commands.");
                    if (i == args.Length || args[i].StartsWith("--", StringComparison.Ordinal))
                ...
                {
                    if (command is not ("baseline-approve" or "baseline-approve-clean" or "baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift" or "baseline-approve-step20-missing-only")) throw new FormatException("--approval-note is only valid with baseline approval commands.");
                    if (i == args.Length || args[i].StartsWith("--", StringComparison.Ordinal))
            ...
            Console.WriteLine(command switch
            {
                ...
                "baseline-review-step19-small-drift" => "Step 19 controlled three-candidate SmallDrift review. Read/report only; no approvals or Solr writes.\n",
                ...
                "baseline-review-step21-small-drift" => "Step 21 controlled four-candidate SmallDrift review. Read/report only; no approvals or Solr writes.\n",
                "baseline-approve-step21-small-drift" => apply
                    ? "Step 21 reviewed bounded SmallDrift approval. Records baseline approval in MariaDB only; no Solr writes.\n"
                    : "Step 21 bounded SmallDrift approval preview. No status or Solr writes.\n",
            ...
            if (command is "baseline-approve" or "baseline-approve-clean" or "baseline-approve-small-drift" or "baseline-approve-step18-small-drift" or "baseline-approve-step19-small-drift" or "baseline-approve-step21-small-drift" or "baseline-approve-step20-missing-only")
            {
            ...
            if (command is "baseline-review-step21-small-drift" or "baseline-approve-step21-small-drift")
            {
                if (candidateId is not null) Step21SmallDriftPolicy.RequireExpansionCandidate(candidateId);
            ...
            if (command == "baseline-review-step19-small-drift")
                return Step19SmallDriftService.Review(settings, connection, workerRoot!, reportDir, candidateId);
            if (command == "baseline-approve-step19-small-drift")
            {
                reviewer ??= Environment.UserDomainName + "\\" + Environment.UserName;
            ...
            if (command == "baseline-review-step21-small-drift")
                return Step21SmallDriftService.Review(settings, connection, workerRoot!, reportDir, candidateId);
            if (command == "baseline-approve-step21-small-drift")
            {
                reviewer ??= Environment.UserDomainName + "\\" + Environment.UserName;
                _ = Step21SmallDriftService.Approve(settings, connection, workerRoot!, candidateId!, baselineSha256!, reviewer, approvalNote, acknowledgeSmallDrift, apply);
                return 0;
            }
    ...
    private static void Help() => Console.WriteLine("""
        WazuhAuditImporter (net9.0)
          ...
          baseline-approve-step18-small-drift --candidate-id <id> Step 18: individual reviewed baseline enrollment only.
                                      --baseline-sha256 <sha> --worker-root <root>
                                      [--reviewer <name>] [--approval-note <text>] [--ack-small-drift] [--apply]
          baseline-review-step19-small-drift --worker-root <root> Step 19: read-only exact review of 1180015,1180021,1180022.
                                      [--candidate-id <id>] [--report-dir <dir>]
          baseline-approve-step19-small-drift --candidate-id <id> Step 19: individual reviewed baseline enrollment only.
                                      --baseline-sha256 <sha> --worker-root <root>
                                      [--reviewer <name>] [--approval-note <text>] [--ack-small-drift] [--apply]
          baseline-review-step21-small-drift --worker-root <root> Step 21: read-only exact review of 1180003,1180006,1180016,1180017.
            [--candidate-id <id>] [--report-dir <directory>]
          baseline-approve-step21-small-drift --candidate-id <id> Step 21: individual bounded SmallDrift enrollment.
            --baseline-sha256 <sha256> --worker-root <root> [--ack-small-drift --apply]
            Requires exact candidate counts, supported extractors, unique scoped paths/IDs and unchanged fingerprint.
