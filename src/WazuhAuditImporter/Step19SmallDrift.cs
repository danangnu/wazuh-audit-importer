...
public sealed record Step19SmallDriftReview(
    string CandidateId,
...
public static class Step19SmallDriftPolicy
{
    ...
    public static void RequireExpansionCandidate(string candidateId)
    {
        if (!IsExpansionCandidate(candidateId))
            throw new EventConflictException(
                $"Step 19 controlled SmallDrift expansion is limited to candidates {string.Join(',', ExpansionCandidates.OrderBy(x => x, StringComparer.Ordinal))}; candidate {candidateId} is outside this controlled batch.");
    }
        ...
        BaselineEnrollmentCapture live)
    {
        ...
        {
            throw new EventConflictException(
                $"Step 19 simple SmallDrift policy rejected candidate {stored.CandidateId}: " +
...
public static class Step19SmallDriftService
{
    public static int Review(
        ...
        string? candidateId)
    {
        settings.ValidateWorker(workerRoot, Path.Combine(Path.GetTempPath(), "wazuh-step19-small-drift-review-validation"));
        var ids = candidateId is null
            ? Step19SmallDriftPolicy.ExpansionBatch.OrderBy(x => x, StringComparer.Ordinal).ToArray()
            : new[] { candidateId };
        var rows = new List<Step19SmallDriftReview>();

        Console.WriteLine("=== Step 19 controlled SmallDrift review ===");
        Console.WriteLine("Controlled expansion batch: 1180015,1180021,1180022 only.");
        ...
        foreach (var id in ids)
        {
            ...
            Step19SmallDriftPolicy.RequireExpansionCandidate(id);
        ...
        Console.WriteLine("STEP 19 SMALLDRIFT REVIEW COMPLETE. Read/report only; no baseline approval and no Solr write occurred.");
        return 0;
    }
    ...
    public static BaselineEnrollmentSnapshot Approve(
        ...
        bool apply)
    {
        settings.ValidateWorker(workerRoot, Path.Combine(Path.GetTempPath(), "wazuh-step19-small-drift-approval-validation"));
        settings.ValidateAllowedCandidate(candidateId);
        Step19SmallDriftPolicy.RequireExpansionCandidate(candidateId);
        ...
        Step19SmallDriftPolicy.RequireSimpleOneForOne(stored, live);
        var review = BuildReview(stored, live);

        Console.WriteLine("Step 19 simple SmallDrift approval preflight.");
        PrintReview(review);
        ...
        if (!acknowledgeSmallDrift)
            throw new FormatException("Step 19 --apply requires --ack-small-drift to confirm the one-missing/one-stale evidence was explicitly reviewed.");

        var defaultNote = $"Step 19 reviewed simple SmallDrift enrollment; missing={live.Evidence.MissingCount}; stale={live.Evidence.StaleCount}; baseline_sha256={stored.BaselineSha256}";
        ...
        Console.WriteLine($"\nSTEP 19 SMALLDRIFT BASELINE APPROVED. candidate={candidateId}; approved_by={approved.ApprovedBy}; sha256={approved.BaselineSha256}");
    ...
    private static Step19SmallDriftReview BuildReview(
        ImportSettings settings,
        MySqlConnection connection,
        string workerRoot,
        string candidateId)
    ...
    public static Step19SmallDriftReview BuildReview(
        BaselineEnrollmentSnapshot stored,
        BaselineEnrollmentCapture live)
    {
        var e = live.Evidence;
        var same = string.Equals(stored.BaselineSha256, live.BaselineSha256, StringComparison.OrdinalIgnoreCase);
        var eligible = Step19SmallDriftPolicy.IsExpansionCandidate(stored.CandidateId) &&
                       Step19SmallDriftPolicy.IsSimpleOneForOneSmallDrift(stored.Status, same,
        ...
        var detail = eligible
            ? "Eligible for the controlled Step 19 one-for-one SmallDrift approval after exact operator review."
            : "Not eligible for the controlled Step 19 approval; keep baseline gated.";
        return new Step19SmallDriftReview(stored.CandidateId, stored.BaselineSha256, same,
    ...
    private static void PrintReview(Step19SmallDriftReview row)
    {
        Console.WriteLine($"candidate={row.CandidateId} sha256={row.BaselineSha256} fingerprint_match={row.FingerprintMatch.ToString().ToLowerInvariant()} " +
                          ...
                          $"missing={row.MissingCount} stale={row.StaleCount} other={row.OtherConflictCount} step19_approval={(row.EligibleForStep19Approval ? "YES" : "NO")}");
    ...
    private static void SaveReports(string? reportDirectory, IReadOnlyList<Step19SmallDriftReview> rows)
    {
        if (string.IsNullOrWhiteSpace(reportDirectory)) return;
        var full = Path.GetFullPath(reportDirectory);
        Directory.CreateDirectory(full);
        var stamp = DateTime.UtcNow.ToString("yyyyMMdd-HHmmss", CultureInfo.InvariantCulture);
        var jsonPath = Path.Combine(full, $"step19-small-drift-review-{stamp}.json");
        var csvPath = Path.Combine(full, $"step19-small-drift-review-{stamp}.csv");
        var mdPath = Path.Combine(full, $"step19-small-drift-review-{stamp}.md");

        File.WriteAllText(jsonPath, JsonSerializer.Serialize(new { GeneratedAtUtc = DateTime.UtcNow, Rows = rows },
            new JsonSerializerOptions { WriteIndented = true }));

        var csv = new StringBuilder();
        csv.AppendLine("candidate_id,baseline_sha256,fingerprint_match,eligible,solr,match,missing,stale,other,step19_approval");
        foreach (var r in rows)
            csv.AppendLine(string.Join(',', new[]
            {
                ...
                Csv(r.EligibleForStep19Approval ? "yes" : "no")
            }));
        File.WriteAllText(csvPath, csv.ToString());

        var md = new StringBuilder();
        md.AppendLine("# Step 19 SmallDrift Review");
        md.AppendLine();
        md.AppendLine("Controlled expansion batch: `1180015`, `1180021`, `1180022`. Review is read-only. Approval does not itself change Solr.");
        foreach (var r in rows)
        {
            md.AppendLine();
            ...
            md.AppendLine($"- Step 19 approval eligible: `{r.EligibleForStep19Approval}`");
